use cova_space::{lattice::Lattice, set::Poset};
...
fn bench_lattice_large(c: &mut Criterion) {
  ...
  group.bench_function("add_relation_500", |b| {
    b.iter(|| {
      let mut lattice = Lattice::new();
      for i in 0..500 {
  ...
  group.bench_function("leq_500", |b| {
    b.iter(|| black_box(large_lattice.leq(black_box(&0), black_box(&250))))
  });
