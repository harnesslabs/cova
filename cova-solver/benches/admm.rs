...
fn bench_admm(c: &mut Criterion) {
  ...
  group.bench_function("lasso_small", |b| {
    b.iter(|| {
  ...
  group.bench_function("lasso_medium", |b| {
  ...
  group.bench_function("basis_pursuit_small", |b| {
    b.iter(|| {
  ...
  group.bench_function("basis_pursuit_medium", |b| {
    b.iter_with_large_drop(|| {
      let n_vars = 10;
...
fn bench_admm_large(c: &mut Criterion) {
  ...
  group.bench_function("lasso_large", |b| {
    b.iter_with_large_drop(|| {
      let n_vars = 50;
      let n_obs = 100;
      let mut a = DMatrix::zeros(n_obs, n_vars);
  ...
  group.bench_function("basis_pursuit_large", |b| {
    b.iter_with_large_drop(|| {
      let n_vars = 50;
