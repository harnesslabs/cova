...
fn bench_linear_programming_large(c: &mut Criterion) {
    ...
    group.bench_function("cost_evaluation_large", |b| b.iter(|| {
        black_box(large_lp.cost(black_box(&large_x)))
    }));

    group.bench_function("gradient_evaluation_large", |b| b.iter(|| {
        black_box(large_lp.gradient(black_box(&large_x)))
    }));

    group.bench_function("solve_large", |b| b.iter(|| {
        black_box(large_lp.solve().unwrap())
    }));
...
fn bench_linear_programming_xlarge(c: &mut Criterion) {
    ...
    group.bench_function("solve_xlarge", |b| b.iter(|| {
        black_box(xlarge_lp.solve().unwrap())
    }));

    group.finish();
}

criterion_group!(benches, bench_linear_programming, bench_linear_programming_large, bench_linear_programming_xlarge);
