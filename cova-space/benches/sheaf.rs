...
fn make_2d_sheaf() -> (
    ...
    let embed_2_to_3 = DMatrix::from_row_slice(3, 2, &[
        1.0, 0.0,
        0.0, 1.0,
        1.0, 1.0,
    ]);
...
fn bench_sheaf_2d(c: &mut Criterion) {
    ...
    group.bench_function("is_global_section_2d", |b| b.iter(|| {
        black_box(sheaf.is_global_section(black_box(&section)))
    }));

    group.bench_function("coboundary_dim0_2d", |b| b.iter(|| {
        black_box(sheaf.coboundary(black_box(0)))
    }));

    group.bench_function("coboundary_dim1_2d", |b| b.iter(|| {
        black_box(sheaf.coboundary(black_box(1)))
    }));
