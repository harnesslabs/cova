...
fn bench_homology(c: &mut Criterion) {
  ...
  let chain =
    Chain::<SimplicialComplex, Boolean>::from_item_and_coeff(&complex, triangle, Boolean(true));

  group.bench_function("boundary", |b| b.iter(|| black_box(chain.boundary())));
...
fn bench_homology_large(c: &mut Criterion) {
  ...
  group.bench_function("boundary_tetrahedron", |b| b.iter(|| black_box(chain_tet.boundary())));
  ...
  group.bench_function("boundary_multi_chain", |b| b.iter(|| black_box(multi_chain.boundary())));
